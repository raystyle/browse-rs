...
- workspace 技能仓（ADR-0008，#50/#51/#56 配套）：知识外置单仓 github.com/raystyle/browse_workspace，部署 `~/.browse-rs/workspace`（BROWSE_WORKSPACE 覆盖，跨实例共享不分 BROWSE_NAME）；`browse workspace status/install/update/list/site/page` 六件（git shell-out，本地修改可 push 回推）加 `add/remove`（#63 自定义仓登记，配置固化 `~/.browse-rs/workspaces.json`，多根序首优先 env 钉死最高）；goto/fetch/detect 回执按域名段与页面特征自动点名（fetch 两腿 CLI 侧点名不依赖引擎，detect 判读映射 page-skill；BROWSE_DOMAIN_SKILLS=0 / BROWSE_PAGE_SKILLS=0 分层关；env 显式透传属防御性，现状与继承等效，改配置重启 daemon）；机制配方与站点知识的增改在 workspace 仓做，不回本仓 docs/skills/（已留指路碑）
