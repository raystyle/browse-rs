...
pub const COMMANDS: &[CmdSpec] = &[
    ...
    CmdSpec {
        name: "issue-new",
        kind: CmdKind::Cli,
        signature: "browse issue new <标题> --acceptance <验收> [--kind bug|improvement] [--body <正文>] [--dry-run]",
        args: &[
            arg!("title", "string", true),
            arg!("acceptance", "string", true),
            arg!("kind", "string", false, "bug"),
            arg!("body", "string", false, "管道 stdin"),
            arg!("dry-run", "boolean", false, "false"),
        ],
        description: "账本 issue 流开单（REQ-063，真源 ledger.ohmygh.com）：acceptance 必填（关单 result 的完成判据锚）；POST 五头签名道，回执 {ok,issue,event}（列表行的 issue_n 见 issue list）；--dry-run 本地校验加载荷预览零网络（账本只增不可撤，契约实弹先走它）。",
        example: "browse issue new \"修复 X\" --kind bug --acceptance \"测试过\" --dry-run",
    },
    CmdSpec {
        name: "issue-list",
        kind: CmdKind::Cli,
        signature: "browse issue list [--limit <n>] [--before <id>]",
        args: &[
            arg!("limit", "number", false, "100"),
            arg!("before", "id", false),
        ],
        description: "列 issue（账本面）：count 是返回条数非在册总数，has_more 权威，打满即 stderr 截断提示。",
    ...
    CmdSpec {
        ...
        description: "看单条 issue 详情（projection 加 timeline）。",
        example: "browse issue show 3",
    },
    CmdSpec {
        name: "issue-close",
        kind: CmdKind::Cli,
        signature: "browse issue close <号> --digest <sha256:...>",
        args: &[
            arg!("n", "number", true),
            arg!("digest", "string", true),
            arg!("note", "string", false),
        ],
        description: "账本关单链（REQ-063）：result 事件引 digest 先行，status=done 收尾，服务端校验。",
        example: "browse issue close 3 --digest sha256:b8277c4fb02b21909d42027ecdce73d1d3bbed001298ca1a2eccafbd892f1444 --note \"随 0.8.0\"",
    },
    CmdSpec {
        name: "artifact-publish",
        kind: CmdKind::Cli,
        signature: "browse artifact publish --name <n> --kind <k> --digest <d> [--dep <d>]…",
        args: &[
            arg!("name", "string", true),
            arg!("kind", "string", true),
            arg!("digest", "string", true),
            arg!("version", "string", false),
            arg!("git-range", "string", false),
            arg!("summary", "string", false),
            arg!("dep", "string", false, "可重复：依赖出处，回溯链即证据链"),
            arg!("outcome", "string", false, "experience 类 success|failure"),
            arg!("git-sha", "string", false, "关联 git sha"),
        ],
        description: "产物共享库发布（REQ-063）：kind 十五枚举必填（binary/experience/lesson/research/prototype/…），digest=正文或记录哈希（sha256:<64hex>），POST 签名道，回 {ok,artifact_id}。",
    ...
    CmdSpec {
        name: "artifact-attest",
        kind: CmdKind::Cli,
        signature: "browse artifact attest <id> --type <t>",
        args: &[
            arg!("id", "string", true),
            arg!("type", "string", false, "attest_dev"),
            arg!("note", "string", false),
        ],
        description: "产物事件（REQ-063 六型）：attest_dev/attest_prod/verification_failed/promote/demote/supersede；id 须 36 字 UUID 形（browse artifact list 取真值）。",
        example: "browse artifact attest 66857bb2-d1ae-4977-97df-8ab41cc37399 --type attest_prod",
    },
    CmdSpec {
        name: "artifact-promote",
        kind: CmdKind::Cli,
        signature: "browse artifact promote <id> [--note <n>]",
        args: &[arg!("id", "string", true), arg!("note", "string", false)],
        description: "提升产物为当前有效（promote 糖，等价 artifact attest <id> --type promote）。",
        example: "browse artifact promote 66857bb2-d1ae-4977-97df-8ab41cc37399",
    },
    CmdSpec {
        name: "artifact-list",
        kind: CmdKind::Cli,
        signature: "browse artifact list [--current] [--env dev|prod]",
        args: &[
            arg!("current", "boolean", false, "false"),
            arg!("env", "string", false),
        ],
        description: "产物列表（GET 免签）：current=1 投影当前有效集，env 过滤。",
        example: "browse artifact list --current --env prod",
    },
    CmdSpec {
        name: "ledger-keygen",
        kind: CmdKind::Cli,
