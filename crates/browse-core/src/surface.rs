...
pub const COMMANDS: &[CmdSpec] = &[
    ...
    CmdSpec {
        ...
        args: &[
            ...
            arg!("fromDir", "string", true),
        ],
        description: "导入安装一个 Chromium 版本到应用数据目录并 pin（ADR-0007 本地面）。",
        example: "browse chrome install 152.0.7977.84 chromium-152.0.7977.84",
    },
    CmdSpec {
        name: "chrome-list",
        kind: CmdKind::Cli,
    ...
    CmdSpec {
        ...
        args: &[arg!("opts", "object", false, "{fromDir}")],
        description: "导入安装 Chromium 版本（version 缺省取目录名；R2 下载腿待 omc 端点）。",
        example: "await chromeInstall({fromDir: \"chromium-152.0.7977.84\"})",
    },
    CmdSpec {
        name: "chromeList",
        kind: CmdKind::Global,
