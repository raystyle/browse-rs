    ...
    async fn call_global(&self, name: &str, argv: &[Value]) -> Result<Value> {
        match name {
            ...
            "subscribeChanges" => {
                ...
                match self.session.call("Browse.subscribeChanges", params).await {
                    Ok(r) => Ok(r),
                    Err(e) => {
                        let msg = format!("{e:#}");
                        if msg.contains("-32601") {
            ...
            "unsubscribeChanges" => {
                let sub = str_arg(argv, 0, "unsubscribeChanges 的 subscriptionId")?;
                match self
                    .session
                    .call(
                        "Browse.unsubscribeChanges",
            ...
            // ---- 引擎层响应等待（#27 批 3）：Browse.waitForResponse 薄封装 ----
            ...
            "engineCancelWait" => {
                let wid = str_arg(argv, 0, "engineCancelWait 的 waitId")?;
                match self
                    .session
                    .call("Browse.cancelWait", json!({ "waitId": wid }))
                    .await
                {
                    Ok(r) => Ok(r),
                    Err(e) => {
            ...
            // ---- 引擎层截图差分（#27 批 4）：Browse.screenshotDiff 薄封装 ----
            "screenshotDiff" => {
                let opts = argv.first().cloned().unwrap_or(json!({}));
                let mut params = json!({});
                if let Some(t) = opts.get("threshold").and_then(Value::as_f64) {
                    params["threshold"] = json!(t);
                }
                // selector 与 semanticSnapshot 同口径（PDL 注释 same as
                // semanticSnapshot；引擎 handler 实作 querySelector 换子树
                // 根，非法/无匹配 -32602 透传归因）
                if let Some(sel) = opts.get("selector").and_then(Value::as_str) {
                    params["selector"] = json!(sel);
                }
                match self.session.call("Browse.screenshotDiff", params).await {
                    Ok(r) => Ok(r),
                    Err(e) => {
                        let msg = format!("{e:#}");
                        if msg.contains("-32601") {
