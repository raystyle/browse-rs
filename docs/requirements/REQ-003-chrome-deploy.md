...
角色裁定（用户令 2026-09-16）：**omc 负责 clean-chrome 发布包的资源分发**（catalog/seed/镜像域）；**browse 自管安装执行**，复制 clean-chrome 既有各版本的安装部署升级形态，不自创机制；**ark 不管 clean-chrome 安装**（fleet 落地执行角色的本仓例外）。

## Criteria

- [ ] 部署：一条命令在 Windows/Linux/macOS 部署 clean-chrome 到本仓应用数据目录（下载走 sha256 边车锚校验）；机制复制 clean-chrome 既有各版本安装部署形态
...
- [ ] 分发源：走 omc 资源分发面（镜像域版本段路由 + `.sha256` 边车锚，口径同 flow-release 第八节）；GitHub 直连仅作回退腿与否随 ADR-0007 定
- [ ] 命令面：新命令登记 `COMMANDS` 单一真相源，schema/llms/skill 三面派生，surface_contract 锁漂移
- [ ] 定标项（开工前问询或派单裁定）：镜像 tool 段命名（与 omc 协调）、版本 pin 载体形态、跨平台资产清单、部署目录命名、托管位发现序优先级
