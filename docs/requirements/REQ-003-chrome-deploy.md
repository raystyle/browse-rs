...
- [x] 升级一键面：`browse chrome update`（方言 `chromeUpdate()`）发现最新版后未装则镜像安装加 pin 切最新，幂等；发现源 `BROWSE_CHROME_LATEST` 钉优先，缺省 `<mirror>/latest.txt` 单行端点（155 前过渡口径，端点候 omc 落桶；2026-09-18 用户令落地）
